...
import { ACCESSORY_CATEGORIES } from "@/lib/navigation";
...
function isAccessoryCategory(category: string): boolean {
  ...
  if (!key) return false;
  if (key.includes("accessor") || key.includes("component")) return true;
  return ACCESSORY_CATEGORIES.some((value: string) => value.toLowerCase() === key);
}
...
function buildAutoProductName(form: ProductFormState) {
  ...
  const categoryLabel = category
    ? isAccessory || /laptop/i.test(category)
      ? category
      : `${category} Laptop`
    : "Laptop";
...
function buildInitialState(initialProduct?: AdminProduct): ProductFormState {
  ...
  return {
    ...
    variants: (initialProduct?.variants || []).map((v: { name: string; price: number; originalPrice?: number; sku?: string }) => ({
      name: v.name || "",
...
export function ProductEditorForm({
    ...
    fetchAdminProducts()
      .then((products: AdminProduct[]) => {
        ...
        const loadedBrands = products
          .map((product: AdminProduct) => product.brand?.trim())
          .filter(Boolean) as string[];
        const loadedCategories = products
          .map((product: AdminProduct) => product.category?.trim())
          .filter(Boolean) as string[];
  ...
  const validate = () => {
    ...
    const isProductWithTechSpecs = !isAccessoryCategory(categoryKey);
  ...
  const addDefaultImage = () => {
    setImageUrls((current) => [...current, "/Hero_Image.png"]);
    setErrors((current) => {
      const next = { ...current };
      delete next.images;
      return next;
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  <span className="text-xs text-gray-400">
                    Accessory quick-pick:
                  </span>
                  {ACCESSORY_CATEGORIES.map((category: string) => (
                    <button
                      key={category}
                      type="button"
                ...
                <div className="space-y-4 rounded-xl border border-gray-200 bg-gray-50/30 p-6">
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-gray-200">
                    <input
                      ...
                      placeholder="Paste image URL (e.g. https://... or /Hero_Image.png)"
                      className="flex-1 min-w-[240px] px-3 py-1.5 text-xs border border-gray-300 rounded-md outline-none focus:border-black"
                    />
                    <Button
                      type="button"
                      size="sm"
                    ...
                    <Button
                      ...
                      className="text-xs h-8 text-gray-600 hover:text-black"
                    >
                      Use Default Laptop Image
                    </Button>
                  </div>
